---
title: "High Performance Computing with R, without rewriting your code"
...
LaptopResults %>% 
  ggplot(aes(x = quasithread, y = time, colour = task, shape = RProvider)) + geom_point() + geom_line() + 
...
Standard R results may not be reliable; didn't have the whole node
These are currently being re-run
...
* Little benefit in > 8 cores anyway
  * `setMKLthreads(<n>)` limits the number of threads used
  * Defaults to whole node
...
* The CSF is ideal for bigger jobs
  * Limit threads used for performance and scheduling reason
